                  ...
                  OrderCloud.Me.ListSpecs(scope.product.ID, null, 1, 100)
                      .then(function(data){
                          ...
                          angular.forEach(data.Items, function(obj, key){
                             obj.DefaultValue? obj.Value = obj.DefaultValue : angular.noop();
                          });
...
function SpecSelectionDirective(Underscore) {
    return {
        scope: {
            spec: '='
        },
        ...
        link: function(scope) {
            ...
            scope.$watch(function() {
                ...
                if (!newVal) return;
                var selectedOption = Underscore.findWhere(scope.spec.Options, {ID : newVal});
