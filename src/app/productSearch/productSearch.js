...
function ProductSearchController($state, OrderCloudParameters, Parameters, ProductList) {
    ...
    vm.updateSort = function(value, order) {
        vm.parameters.sortBy = order ? order + value : value;
        vm.filter(false);
    };
...
function ProductSearchDirectiveController($state, $timeout, $scope, OrderCloud) {
    var vm = this;
    var searching;
    vm.searchResults = [];
...
function ProductSearchModalController($uibModalInstance, $timeout, $scope, OrderCloud) {
    ...
    vm.getSearchResults = function() {
        return OrderCloud.Me.ListProducts(vm.searchTerm, 1, 10)
            .then(function(data) {
                vm.searchResults = data.Items;
            });
    };
